...
Write-Host " 月報完整流程 $(if($Fast){'⚡快速模式'}else{})" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

# 計算月報日期
$today = Get-Date
$firstOfThisMonth = Get-Date -Year $today.Year -Month $today.Month -Day 1
$sd = $firstOfThisMonth.AddMonths(-1)
$ed = $firstOfThisMonth.AddDays(-1)
...
if ($Fast) {
    & powershell -NoProfile -ExecutionPolicy Bypass -File "$base\run_download_and_convert.ps1" -type monthly -Fast
} else {
    & powershell -NoProfile -ExecutionPolicy Bypass -File "$base\run_download_and_convert.ps1" -type monthly
}

Write-Host "`n✓ 月報完整流程完成！" -ForegroundColor Green
Write-Host "📊 輸出: E:\88. Claude\11_inspdata_monthly\insprawdata_monthly" -ForegroundColor Cyan