...
| 操作 | 快捷鍵 | 說明 |
|------|--------|------|
| 重新載入頁面 | `Ctrl+F5` | 清除快取、載入最新資料 |
| 列印為 PDF | `Ctrl+P` | 列印或另存為 PDF |
| 開發者工具 | `F12` | 檢查 HTML / CSS |
| 全螢幕模式 | `F11` | 全螢幕查看儀表板 |
| 返回上一頁 | `Alt+←` | 返回總頁 |
| 尋找文字 | `Ctrl+F` | 搜尋訂單或指標 |
...
| 問題 | 症狀 | 原因 | 解決方案 |
|------|------|------|---------|
| 資料過期 | 數字不更新 | 瀏覽器快取 | 按 `Ctrl+F5` 強制重載 |
| 圖表不顯示 | 只看到空白區域 | 圖片 Base64 編碼損壞 | 重新上傳圖表，更新 Base64 碼 |
| 表格亂碼 | 中文顯示為方塊或符號 | 字元編碼問題 | 檢查 HTML 的 `<meta charset="utf-8">` |
| 連結失效 | 點選無反應或 404 | 檔案名稱改變或移動 | 檢查 `href` 中的檔案路徑是否正確 |
| 瀏覽器閃爍 | 頁面不斷重載 | JavaScript 錯誤 | 按 `F12` 查看 Console 錯誤訊息 |
| 列印問題 | 列印時內容被裁切 | 邊距設定過大 | 列印前調整邊距（通常設為 0.5 吋） |
| 行動版顯示異常 | 手機上排版亂掉 | CSS 響應式設定不足 | 在 `<style>` 中新增 `@media` 查詢 |
...
1. 確認 `12_inspdata_weekly\analysis chart_weekly_*.xlsx` 存在
2. 開啟 Excel，檢查該分店的工作表是否有資料
3. 若工作表存在但為空，檢查週報導入是否成功
4. 在 `weekly-report` Skill 中手動執行 `run_weekly_full.ps1 -Fast`

### 問題：所有分店資料都是舊的

**檢查步驟**：

1. 查詢今天的日期：`Get-Date -Format "yyyy-MM-dd"`
2. 進到 `12_inspdata_weekly` 資料夾
3. 檢查最新的 `analysis chart_weekly_*.xlsx` 檔案的修改時間
4. 若時間是昨天或更早，執行 `weekly-report` Skill 更新資料
...
2. **清除瀏覽器快取**（定期）：
   ```
   Chrome: Ctrl+Shift+Delete → 清除瀏覽資料
   Firefox: Ctrl+Shift+Delete → 清除近期歷史記錄
   ```
3. **減少開啟的分頁**：每個分頁會佔用記憶體
4. **使用硬體加速**：在瀏覽器設定中啟用
...
1. 壓縮內嵌的 PNG 圖片（使用 TinyPNG.com）
2. 移除不必要的 CSS 和 JavaScript
3. 最小化 HTML（移除空格和註解）
