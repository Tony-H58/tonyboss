...
## 📋 目錄
1. [開發流程規則](#開發流程規則)
2. [自訂 Skill 一覽](#自訂-skill-一覽)
3. [系統工作流程對應](#系統工作流程對應)
4. [各 Skill 詳細說明](#各-skill-詳細說明)
5. [常見工作場景](#常見工作場景)
6. [快速參考](#快速參考)
...
同樣的規則也寫在專案根目錄的 `CLAUDE.md`（Claude Code 會自動載入）：

1. **寫程式前** 先查 `.claude/skills/` 與本指南，確認是否已有涵蓋此需求的 skill，避免重複造輪子。
2. **程式完成後** 評估是否該用 `skill-creator` 新增 skill，或更新既有 skill 的 `SKILL.md`。
3. **只有使用者明確說「儲存」時**，才把新增/更新內容實際寫入 `.claude/skills/`，並同步更新本指南與 `Claude Skills 總表.md`。在此之前只在對話中提出草稿。
